...
public class SkillFactory {
    ...
    public static void loadDelays() {
        DatabaseConnection.domain(con -> {
            if (WzSqlName.wz_delays.check(con)) {//load from sql
                ...
                WzSqlName.wz_delays.update(con);
            }
            return null;
        });
    }
                        ...
                        if (data.getName().equals("skill")) {
                            for (MapleData data2 : data) { // Loop thru each jobs
                                if (data2 != null) {
                                    ...
                                    summon_data = null;
                                    if (skillid == 聖魂劍士.極樂之境_2 || skillid == 351110002 || skillid == 陰陽師.式神炎舞_1 || skillid == 夜使者.達克魯的秘傳) {
...
//                                    sse.delay = MapleDataTool.getInt("bulletSpeed", summon_data, 0);
        ...
        log.info("共載入 " + skills.size() + " 個技能資訊");
    }
